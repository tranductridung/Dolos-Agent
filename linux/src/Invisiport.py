...
class Invisiport(Core):
    ...
    def __add_blacklist(self, ip):
        with db_helper('invisiport_blacklist.db') as c:
            try:
                ...
                super().log(Core.INFO, "Add IP {} to blacklist".format(ip))
            except Exception as e:
                super().log(Core.WARNING, 'exception occured {}'.format(e))
    ...
    def __blacklist(self, ip, port):
        ...
        super().log(Core.WARNING, "Someone connect to Invisiport")
        
        params = "sudo iptables -A INPUT -s {} -p tcp ! --destination-port {} -j DROP".format(ip, Invisiport.PORT)
        subprocess.run(params,shell=True)
        super().log(Core.INFO, "Add rule to block IP {} in iptables".format(ip))
    
        params = "sudo iptables -t nat -A PREROUTING -s {} -p tcp --dport {} -j REDIRECT --to-port {}".format(ip, port, Invisiport.PORT)
        subprocess.run(params, shell=True)
        super().log(Core.INFO, "Add rule to redirect packet from IP {} to port 443".format(ip))
